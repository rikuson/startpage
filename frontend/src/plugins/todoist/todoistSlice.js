...
export const readProjects = createAsyncThunk(
  'todoist/readProjects',
  async (_, thunk) => {
    const state = thunk.getState();
    const api = new Rest(state.todoist.token);
...
const todoistSlice = createSlice({
  ...
  extraReducers: {
    ...
    [readProjects.fulfilled]: (state, action) => {
      ...
      if (
        !state.projects.some((project) => project.id === state.activeProject)
      ) {
...
export const { removeToken, openProject } = todoistSlice.actions;
