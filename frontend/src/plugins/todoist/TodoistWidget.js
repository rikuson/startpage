...
class TodoistWidget extends Component {
  ...
  render() {
    return (
      <TaskList tasks={this.props.tasks} project={this.props.activeProject} onChange={e => this.handleChange(e)} onDelete={e => this.handleDelete(e)} />
    );
  }
}
    ...
    <ul className="list-group list-group-flush">
      {props.tasks.filter(t => t.project_id === props.project).sort((a, b) => a.order < b.order ? 1 : -1).map(task => (
        <li key={task.id} className="list-group-item d-flex justify-content-between align-items-center">
          ...
          <button type="button" className="btn btn-sm btn-outline-danger" onClick={() => props.onDelete(task.id)}><FontAwesomeIcon icon={faTrash} onClick={() => props.onDelete(task.id)} /></button>
        </li>
      ))}
    </ul>
  );
